...
features=pd.read_csv('/home/ujjwal/Documents/IIT_KGP_Internship/DataSet/CSV_File/Flu1.csv')
length_fet=len(features)
pred=[]
pre=[]
for i in training_iterations:
    ...
    #   placeholder values
    hyps, evis, ys = (data_feature_list[0][batch,:],data_feature_list[1][batch,:],correct_scores[batch])
...
for j in range(len(features)):        
    evidences = [features['Text'][j]]
    
    hypotheses = ["vaccination cures flu"]
    ...
    pre.append(["Positive", "Neutral", "Negative"][np.argmax(prediction[0])]+" entailment")
    pred.append(prediction[0])
    
file = open('/home/ujjwal/Documents/IIT_KGP_Internship/Output/TE_Data/TE1_flu.csv','a')
fields = ('sentences','hypotheses','pos_scr','neg_scr','nut_scr','te')
wr = csv.DictWriter(file, fieldnames=fields, lineterminator = '\n')
wr.writeheader()
for i in range(length_fet):
            wr.writerow({'sentences':features['Text'][i],'hypotheses':hypotheses,'pos_scr':pred[i][0],'neg_scr':pred[i][2],'nut_scr':pred[i][1],'te':pre[i]}) 
